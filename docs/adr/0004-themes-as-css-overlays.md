...
A Theme is a folder containing a `theme.css` file. The folder name is the Theme's ID. Themes are resolved across two scopes — Global (app data directory) and Root (`.config/themes/[ID]/`) — with the Root scope taking precedence on ID collision.
