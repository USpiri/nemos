...
- **Table column alignment**: Tables now preserve left/center/right column alignment when opening and saving a Note — previously parsed from the file and silently discarded. Table cells are also now restricted to a single line of inline content. ([#58](https://github.com/USpiri/nemos/issues/58))
